...
## this is the overlay to average of a1 a2
...
for lm in loss_modes:
    fn = f'predict/testepoch/predict_a1_calc_mass_regr_loss{lm}_epoch20.root'
    df = pd.DataFrame()
    f = uproot.open(fn)
    g = f['Events']
...
for pltobj, centralval, wideval, savename, yrange in zip([mmslog, rmslin, rmslog, senplt],
    ...
    plt.grid()
    pltobj[1].legend()
    pltobj[1].set_title(f'{savename} wideH regression to avg of a1+a2 and central a1, mass loss{lm}')
    pltobj[1].set_ylim([yrange[0], yrange[1]])
    pltobj[0].savefig(f'plots/wideH_aMass_overlay/{savename}_avg_a1a2_mass_loss{lm}.png', bbox_inches='tight')
    plt.close(pltobj[0])



    
# Regressions to mass of a1
#Overlay 4 different trainings (bb_mass, bb_dR, bbbar_mass, bbbar_dR)
#Make separate sets of plots for "output vs. target_mass" and "output vs. fj_gen_H_aa_bbbb_mass_a1"
for flist_a, apart, clipvals in zip ([flist_a1, flist_a2], ['a1', 'a2'], [(0,150), (0,80)]):    
    ...
    for df, fname in zip(dfswide, flist_a) :            
        ...
        target_massa1_plt[1].set_xlabel('fj_gen_H_aa_bbbb_mass_a1')
...
##  * a2_output_vs_target
for fa1, fa2 in zip(flist_a1, flist_a2):
    a1df = pd.DataFrame()
    a1g = uproot.open(fa1)['Events']
    for v in vs:
        a1df[v] = np.array(a1g[v].array())

    a2df = pd.DataFrame()
    a2g = uproot.open(fa2)['Events']
    for v in vs:
        a2df[v] = np.array(a2g[v].array())

    for df, apart in zip([a1df, a2df], ['a1', 'a2']):
        df['ratio'] = np.divide(df['output'], df['target_mass'])
        df['log2ratio'] = np.clip(np.log2(df['ratio']), a_min=-2, a_max=2)
        df['genMass_ratio'] = np.divide(df['output'], df[f'fj_gen_H_aa_bbbb_mass_{apart}'])
        df['genMass_log2ratio'] = np.clip(np.log2(df['genMass_ratio']), a_min=-2, a_max=2)
    ...
    rmslin_wide_vstarget_list = []
    senlin_wide_vstarget_list = []
    mmslog_wide_vstarget_list = []
    rmslog_wide_vstarget_list = []
    
    rmslin_wide_vsgenMass_list = []
    senlin_wide_vsgenMass_list = []
    mmslog_wide_vsgenMass_list = []
    rmslog_wide_vsgenMass_list = []
    
    ## rms mms and such    
    fcentral = f'predict/testepoch/predict_a1_calc_mass_regr_loss0_epoch20.root'
    gcentral = uproot.open(fcentral)['Events']
    dfcentral = pd.DataFrame()
    for v in vs:
        dfcentral[v] = np.array(gcentral[v].array())
    for lowermass, uppermass in zip(massranges[:-1], massranges[1:]):
        dfwideclipped = df[(df['target_mass'] >= lowermass) & (df['target_mass'] < uppermass)]
        dfcentralclipped = dfcentral[(dfcentral['target_mass'] >= lowermass) & (dfcentral['target_mass'] < uppermass)]
        ...
        rmslin_wide_vstarget_list.append(np.std(dfwideclipped['ratio']))
        mmslog_wide_vstarget_list.append(np.mean(dfwideclipped['log2ratio']))
        rmslog_wide_vstarget_list.append(np.std(dfwideclipped['log2ratio']))
        
        s_hist, s_edge = np.histogram(dfwideclipped['ratio'], bins=101, range=(0,2.02))
        binwidth = s_edge[1]-s_edge[0]
        sensitivity = np.sum(s_hist[:-1]*s_hist[:-1])*100/(np.square(np.sum(s_hist)))
        senlin_wide_vstarget_list.append(sensitivity)

        rmslin_wide_vsgenMass_list.append(np.std(dfwideclipped['genMass_ratio']))
        mmslog_wide_vsgenMass_list.append(np.mean(dfwideclipped['genMass_log2ratio']))
        rmslog_wide_vsgenMass_list.append(np.std(dfwideclipped['genMass_log2ratio']))
        
        s_hist, s_edge = np.histogram(dfwideclipped['genMass_ratio'], bins=101, range=(0,2.02))
        binwidth = s_edge[1]-s_edge[0]
        sensitivity = np.sum(s_hist[:-1]*s_hist[:-1])*100/(np.square(np.sum(s_hist)))
        senlin_wide_vsgenMass_list.append(sensitivity)
    ...
    for pltobj, centralval, wideval_vstarget, wideval_vsgenMass, savename, yrange in zip([mmslog, rmslin, rmslog, senplt],
                                                                             #[mmslogtestlist, rmslintestlist, rmslogtestlist, senlintestlist],
                                                                             #[mmslogtrainlist, rmslintrainlist, rmslogtrainlist, senlintrainlist],
                                                                             #[mmslo g_central_list, rmslin_central_list, rmslog_central_list, senlin_central_list],
                                                                             [mmslog_wide_vstarget_list, rmslin_wide_vstarget_list, rmslog_wide_vstarget_list, senlin_wide_vstarget_list],
                                                                             [mmslog_wide_vsgenMass_list, rmslin_wide_vsgenMass_list, rmslog_wide_vsgenMass_list, senlin_wide_vsgenMass_list],
                                                                             ['mms log ratio', 'rms ratio', 'rms log ratio', 'sensitivity'],
                                                                             [(-.5, .4), (0, .4), (0,.5), (0,10)]):
        ...
        arr = list(range(len(xticks)))
        # pltobj[1].plot(centralval, label='central')
        pltobj[1].plot(wideval_vstarget, label='vs target')
        pltobj[1].plot(wideval_vsgenMass, label='vs genMass')
        pltobj[1].set_xticks(arr, xticks,)
        pltobj[1].set_xlabel('mass points')
        plt.grid()
        pltobj[1].legend()
        pltobj[1].set_title(f'{savename} ')
        pltobj[1].set_ylim([yrange[0], yrange[1]])
        pltobj[0].savefig(f'plots/wideH_aMass_overlay/{savename}_.png', bbox_inches='tight')
        plt.close(pltobj[0])
