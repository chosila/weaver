#### THIS IS THE SAME AS THE OTHER FILE BUT SAMPLES ARE PREDICTED USING WH-70 SINTEAD OF CENTRAL
#### PLOT NAMES CHANGED BUT THAT"S IT 
...
for pltobj, centralval, wideval, savename, yrange in zip([mmslog, rmslin, rmslog, senplt],
    ...
    plt.grid()
    pltobj[1].legend()
    pltobj[1].set_title(f'{savename} wideH (wH-70) regression to avg of a1+a2 and central a1, mass loss3')
    pltobj[1].set_ylim([yrange[0], yrange[1]])
    pltobj[0].savefig(f'plots/wideH_aMass_overlay/{savename}_avg_a1a2_mass_loss3_wH-70.png', bbox_inches='tight')
    plt.close(pltobj[0])
